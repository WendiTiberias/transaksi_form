<?php 
 ...
 $tbbeli = mysqli_query($conn , "SELECT * FROM tbbeli WHERE no='$no'");
 while($resi = mysqli_fetch_array($tbbeli)) {
    $total = $resi['grandtotal'];
    $subtotal = $resi['subtotal'];
 }
 
 $tempbeli = mysqli_query($conn , "SELECT * FROM tempbelidetil WHERE no='$no'");
...
$tbbarang = mysqli_query($conn , "SELECT * FROM tbbarang WHERE kodebarang='$kodebarang'");
 ...
 $db = mysqli_query($conn , "INSERT INTO tbstokpembelian (Tgl,nobeli,suplier,user,nama,jlh,harga,total,subtotal) VALUES('$tgl','$no','$suplier','$user','$nama','$jlh','$harga','$total','$subtotal')");
 ...
 <table class="w-4/5 mx-auto mt-10 table-fixed">
  ...
  <tbody>
    <?php 
      ...
      include "../koneksi.php";
      $db = mysqli_query($conn , "SELECT * FROM tbstokpembelian");
      $counter = 0;
